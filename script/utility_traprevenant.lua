...
function TrapRevenant.MakeSetTurnCost(id)
	return function(e,tp,eg,ep,ev,re,r,rp,chk)
		...
		if not c:IsStatus(STATUS_SET_TURN) then
	...
	if #g>0 and Duel.SendtoGrave(g,REASON_EFFECT)>0 then
		...
		if Duel.GetLocationCount(tp,LOCATION_SZONE)>0
			...
			if sc and Duel.SSet(tp,sc)>0 then
				local e1=Effect.CreateEffect(e:GetHandler())
				e1:SetDescription(aux.Stringid(TrapRevenant.SHROUD_ID,3))
...
function TrapRevenant.absolutionval(e,re,val,r,rp,rc)
	local hp=e:GetHandlerPlayer()
	if rp==hp and val>0 then
		Duel.Damage(1-hp,val,REASON_EFFECT)
		return 0
	end
	return val
end
function TrapRevenant.ResolveAbsolution(e,tp)
	--EFFECT_CHANGE_DAMAGE handles battle damage, and is cloned onto
	--EFFECT_NO_EFFECT_DAMAGE (same value function) to also cover effect
	--damage, mirroring how official "take no damage" cards are scripted.
	local e1=Effect.CreateEffect(e:GetHandler())
	e1:SetType(EFFECT_TYPE_FIELD)
	e1:SetCode(EFFECT_CHANGE_DAMAGE)
	e1:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e1:SetTargetRange(1,0)
	e1:SetValue(TrapRevenant.absolutionval)
	e1:SetReset(RESET_PHASE|PHASE_END)
	Duel.RegisterEffect(e1,tp)
	local e2=e1:Clone()
...
function TrapRevenant.ResolveRecall(e,tp)
	...
	if #g>0 and Duel.SSet(tp,g)>0 then
		for tc in g:Iter() do
			local e1=Effect.CreateEffect(e:GetHandler())
			e1:SetDescription(aux.Stringid(TrapRevenant.RECALL_ID,2))
			e1:SetType(EFFECT_TYPE_SINGLE)
			e1:SetProperty(EFFECT_FLAG_SET_AVAILABLE)
			e1:SetCode(EFFECT_TRAP_ACT_IN_SET_TURN)
			e1:SetReset(RESET_EVENT|RESETS_STANDARD)
